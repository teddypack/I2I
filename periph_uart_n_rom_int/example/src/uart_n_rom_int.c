/*
 * @brief UART ROM API interrupt example
 *
 * @note
...
static void setupUART()
{
	...
	if (frg_mult) {
		/* Caution: Will alter all UART N multiplier rates */
		LPC_SYSCTL->FRGCLKDIV = frg_mult;
	}
}
...
{
	LPC_UARTND_API->uart_isr(uartHandle);
}
...
int main(void)
{
	...
	/* Transmit the welcome message and instructions using the
	   putline function */
	putLineUART("LPC11U6X USART ROM API interrupt Example\r\n");
	...
	while (indexIn < 16) {
		...
		LPC_UARTND_API->uart_put_char(uartHandle, recv_buf[indexIn]);
