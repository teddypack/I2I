...
static bool dmaTXSend(uint8_t *data, int bytes)
{
	...
	dmaTXDesc[countTXDescUsed].dest = DMA_ADDR(&LPC_USART0->RBR);	/* Byte aligned */
...
static void dmaRXQueue(void)
{
	...
	for (i = 0; i < UARTRXDESC; i++) {
		...
		/* Create a descriptor for the data */
		dmaRXDesc[i].source = DMA_ADDR(&LPC_USART0->RBR) + 0;	/* Byte aligned */
...
int main(void)
{
	...
	/* Enable FIFOs and DMA support */
	Chip_UART0_SetupFIFOS(LPC_USART0, (UART0_FCR_FIFO_EN | UART0_FCR_TRG_LEV0 | UART0_FCR_DMAMODE_SEL));
