...
int main(void)
{
	...
	Chip_SYSCTL_SetBODLevels(SYSCTL_BODRSTLVL_LEVEL0, SYSCTL_BODINTVAL_2_LEVEL3);
	...
	if ((sysResetStatus & SYSCTL_RST_BOD) == 0) {
		...
		/* Board was reset via a BOD eventt */
		sysResetStatus |= SYSCTL_RST_BOD;
	}

	/* Clear reset status */
	if (sysResetStatus & SYSCTL_RST_BOD) {
		/* This will likely never happen, as the chip will lose most status
